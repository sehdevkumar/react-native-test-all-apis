...
const IndexPage = () => {
  ...
  }, [selectedIndex, selectedIndexMonth, selectedIndexWeek]);
      ...
      <View style={{ padding: 10 }}>
        ...
        >
          ...
          <Select
            ...
            style={{ backgroundColor: Colors.dark.background, flex: 1 }}
            value={weeks[(selectedIndexWeek as IndexPath).row]}
            selectedIndex={selectedIndexWeek}
            onSelect={(index) => setSelectedIndexWeek(index)}
          >
            {generateWeeks?.map((week) => (
              <SelectItem key={week.weekNumber} title={'Week ' + week.weekNumber.toString()} />
            ))}
          </Select>
        </View>
      </View>
